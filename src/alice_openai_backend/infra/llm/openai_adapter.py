...
from alice_openai_backend.domain.models import ConversationTurn, FollowupMode, LLMReply, TurnRole
...
_FOLLOWUP_MARKER_PATTERN = re.compile(
    r"\[\[FOLLOWUP_REQUEST:\s*(?P<request>.*?)\s*\]\]",
    re.DOTALL,
)
...
def _build_input_items(
    *,
    ...
    input_items: list[dict[str, object]] = [
        {
            "role": "developer",
            "content": _build_developer_prompt(system_prompt, utc_now=utc_now),
        }
    ]
    for turn in history[-8:]:
        ...
        if turn.role == TurnRole.SYSTEM:
            input_items.append(
                {
                    "role": "developer",
                    "content": _format_historical_system_turn(turn.content),
                }
            )
            continue
