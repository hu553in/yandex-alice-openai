...
class RedisStreamQueue:
    def __init__(
        self, redis: Redis[str], keys: RedisKeyspace, *, reclaim_idle_ms: int, poll_timeout_ms: int
    ) -> None:
        self._redis = redis
        self._keys = keys
    ...
    async def ensure_group(self) -> None:
        try:
            await self._redis.xgroup_create(
                self._keys.stream(), self._consumer_group, id="$", mkstream=True
            )
    ...
    async def ack(self, stream_id: str) -> None:
        await self._redis.xack(self._keys.stream(), self._consumer_group, stream_id)
