...
class RedisPendingReplyStore:
    ...
    async def start_pending(self, conversation_key: str, job: DeferredJob) -> PendingReply:
        ...
        created = await self._redis.set(
            key, _pending_to_json(payload), ex=self._keys.pending_ttl_seconds, nx=True
        )
        if created:
            return payload
...
class RedisIdempotencyStore:
    ...
    async def store_response(self, key: str, response: dict[str, object]) -> None:
        await self._redis.set(
            self._keys.idempotency(key), json.dumps(response), ex=self._keys.idempotency_ttl_seconds
        )
