...
from yandex_alice_openai.domain.ports import (
    AnalyticsSink,
    ConversationStore,
    IdempotencyStore,
    JobQueue,
...
from yandex_alice_openai.services.prompting import is_continue_intent, sanitize_user_text
...
class ConversationService:
    ...
    async def handle(self, payload: AliceWebhookRequest) -> AliceWebhookResponse:
        ...
        if not utterance:
            ...
            await self._idempotency_store.store_response(
                payload.request_key(), response.model_dump()
            )
            return response
        ...
        if pending is not None:
            blocking_response = await self._handle_active_pending(
                payload, scope.conversation_key, pending
            )
            if blocking_response is not None:
                await self._idempotency_store.store_response(
                    payload.request_key(), blocking_response.model_dump()
                )
    ...
    async def _handle_active_pending(
        self, payload: AliceWebhookRequest, conversation_key: str, pending: PendingReply
    ...
    async def _handle_continue(
        self, payload: AliceWebhookRequest, conversation_key: str, pending: PendingReply | None
    ) -> AliceWebhookResponse:
        if pending is None:
            return AliceWebhookResponse(
                session=payload.session,
        ...
        if (
            ...
            return await self._defer_request(
                payload, conversation_key, pending.continuation_request
            )
    ...
    async def process_deferred_job(self, job: DeferredJob, *, deadline_seconds: float) -> LLMReply:
        ...
        reply = await self._generate_deferred_reply(
            job=job, history=history, deadline_seconds=deadline_seconds
        )
        ...
        if updated is None:
            self._logger.warning(
                "deferred_job_stale", job_id=job.job_id, conversation_key=job.conversation_key
            )
            return reply
        await self._persist_analytics(
            job.conversation_key, [user_turn, assistant_turn], job, reply, None
        )
        return reply

    async def fail_deferred_job(self, job: DeferredJob, *, error_message: str) -> None:
        await self._pending_store.mark_failed(
            job.conversation_key, job_id=job.job_id, error_message=error_message
        )
    ...
    async def _generate_deferred_reply(
        self, *, job: DeferredJob, history: Sequence[ConversationTurn], deadline_seconds: float
    ) -> LLMReply:
    ...
    async def _defer_request(
        self, payload: AliceWebhookRequest, conversation_key: str, utterance: str
    ) -> AliceWebhookResponse:
        job = DeferredJob(
            job_id=uuid4().hex,
            request_id=payload.request_key(),
        ...
        return AliceWebhookResponse(
            session=payload.session,
            response=render_voice_response(
                "Я готовлю ответ. Скажи: продолжай.", buttons=[AliceButtonsItem(title="Продолжай")]
            ),
        )

    async def _record_completed_turns(
        self, conversation_key: str, user_turn: ConversationTurn, assistant_turn: ConversationTurn
    ) -> None:
        await self._append_turns(conversation_key, user_turn, assistant_turn)
        await self._persist_analytics(
            conversation_key, [user_turn, assistant_turn], None, None, None
        )
