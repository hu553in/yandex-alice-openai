...
from yandex_alice_openai.schemas.alice import AliceButtonsItem, AliceResponsePayload
...
def render_voice_response(
    text: str, *, end_session: bool = False, buttons: list[AliceButtonsItem] | None = None
) -> AliceResponsePayload:
    normalized = _normalize_text(text)
    clipped = _clip(normalized, limit=MAX_ALICE_FIELD_LENGTH)
    return AliceResponsePayload(
        text=clipped, tts=clipped, end_session=end_session, buttons=buttons or []
    )
